...
def _document_to_row(doc) -> dict:
    """Normalizes a databases.list_documents() result item into a plain dict of
    field -> value, regardless of whether the SDK handed back a raw dict (older
    SDKs) or a Document model (appwrite-py 24.0.0+, whose custom fields live
    under .data and are not subscriptable on the Document itself)."""
    if isinstance(doc, dict):
        row = dict(doc.get("data", doc))
        row["$id"] = doc.get("$id", row.get("$id"))
        return row

    data = doc.data
    row = dict(data) if isinstance(data, dict) else dict(vars(data))
    row["$id"] = doc.id
    return row
...
def load_upcoming_fixtures(databases: Databases, context) -> list:
    ...
    while True:
        ...
        if not docs:
            break
        rows.extend(_document_to_row(d) for d in docs)
        if len(docs) < 100:
            break
